...
let playables = [];
let fetchedAt = 0;
export function parsePlayables(raw) {
    // The reply is a log line, then one comma-separated row with a trailing comma.
    const line = raw.split('\n').find((l) => l.includes(',')) ?? '';
    return [...new Set(line
            .split(',')
            .map((s) => s.trim())
            .filter((s) => /^[A-Za-z]{3,}$/.test(s)))].sort((a, b) => a.localeCompare(b));
}
export async function speciesList(ctx) {
    if (playables.length > 0 && Date.now() - fetchedAt < PLAYABLES_TTL_MS)
        return playables;
    try {
...
export function mutationList(ctx) {
 ...
 */
export function suggest(options, typed) {
