...
export function noticeStyle(ctx) {
    return ctx.db.getSetting(STYLE_KEY) === 'persistent' ? 'persistent' : 'banner';
}
export const setNoticeStyle = (ctx, style) => ctx.db.setSetting(STYLE_KEY, style);
/**
 ...
 */
export async function tell(ctx, steamId, message, { persist = false } = {}) {
    const style = persist ? 'persistent' : noticeStyle(ctx);
    try {
        if (style === 'persistent')
            return await ctx.mod.notify(steamId, message);
 ...
 */
export async function tellEveryone(ctx, message, { persist = false } = {}) {
    let players = [];
    try {
    ...
    for (const player of players) {
        if (await tell(ctx, player.steamId, message, { persist }))
            told += 1;
    }
    return told;
}
