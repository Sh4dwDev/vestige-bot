...
const describe = (c) => (c.label ?? `Condition ${c.index} _(not yet identified)_`);
...
export function buildPrimeEmbed(state, ctx) {
    ...
    if (state.eligible) {
        return embed.setDescription(`Every condition is met — you are eligible for Prime on ${SERVER}.\n\n` +
            `**${met.length} of ${conditions.length}** conditions met.`);
    }
    embed.setDescription(`**${met.length} of ${conditions.length}** conditions met. ` +
    ...
    if (unnamed > 0) {
        embed.addFields({
            ...
            value: `The game exposes these as ten unlabelled flags, and **${unnamed}** of ` +
...
export function buildPrimeDebugEmbed(state, who) {
    ...
    return new EmbedBuilder()
        ...
        .setDescription(`For ${who}. Change one thing in game, run this again, and see which ` +
        'flag moved — that is how a condition gets a name it deserves.')
        .addFields({
        name: 'Conditions',
        value: conditions.length === 0
            ? '_The pawn exposed none._'
            : conditions.map((c) => `\`${String(c.index).padStart(2)}\` ${c.met ? '✅' : '❌'}` +
