 ...
 */
const KEEP = 14;
export function backupConfig(config) {
 ...
 */
export async function runBackup(ctx, cfg) {
    ...
    try {
        ...
        for (const table of tables) {
            ...
            rows += data.length;
            await conn.execute('INSERT INTO vesta_backups (taken_at, table_name, rows_json, row_count) VALUES (?, ?, ?, ?)', [takenAt, table, JSON.stringify(data), data.length]);
        }
        ...
        const [old] = await conn.query('SELECT DISTINCT taken_at FROM vesta_backups ORDER BY taken_at DESC LIMIT ?, 18446744073709551615', [KEEP]);
...
export async function listSnapshots(cfg) {
    const conn = await mysql.createConnection({
    ...
    try {
        await conn.query(SCHEMA);
        const [rows] = await conn.query(`SELECT taken_at, COUNT(*) AS tables_count, SUM(row_count) AS rows_count
 ...
 */
export async function restoreSnapshot(ctx, cfg, takenAt) {
    const conn = await mysql.createConnection({
        host: cfg.host, port: cfg.port, user: cfg.user,
        password: cfg.password, database: cfg.database,
    });
    try {
        const [rows] = await conn.query('SELECT table_name, rows_json FROM vesta_backups WHERE taken_at = ?', [takenAt]);
        if (rows.length === 0)
            throw new Error(`no snapshot taken at ${takenAt}`);
        let restored = 0;
        for (const row of rows) {
            const table = String(row['table_name']);
            const data = JSON.parse(String(row['rows_json']));
...
export function lastBackup(ctx) {
    return Number.parseInt(ctx.db.getSetting(LAST_KEY) ?? '', 10) || 0;
}
export function markBackup(ctx, at) {
