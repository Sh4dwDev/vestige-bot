...
import { slotInfo } from './market.js';
import { apiError, displayPoints, originAllowed, toVaultSlot, } from './webapi.js';
import { clearCookie, newSecret, newState, parseCookies, readSession, safeReturnPath, serializeCookie, SESSION_COOKIE, SESSION_TTL_MS, signSession, STATE_COOKIE, STATE_TTL_MS, } from './websession.js';
/**
 * The website API.
 *
...
const sendJson = (res, status, body, headers = {}) => send(res, status, JSON.stringify(body), {
 ...
 * act on.
 */
async function identify(ctx, code, redirectUri) {
    const web = ctx.config.web;
    if (!web)
        throw new Error('the website is not configured');
    ...
    if (!token.ok)
        throw new Error(`token exchange failed with ${token.status}`);
    const granted = (await token.json());
    if (typeof granted.access_token !== 'string')
        throw new Error('no access token in the reply');
    const me = await fetch(ME_URL, {
        headers: { authorization: `Bearer ${granted.access_token}` },
    });
    if (!me.ok)
        throw new Error(`identify failed with ${me.status}`);
    const user = (await me.json());
    if (typeof user.id !== 'string')
        throw new Error('no user id in the reply');
    return user.id;
}
/**
 ...
 */
async function readVault(ctx, steamId, cache) {
    const hit = cache.get(steamId);
    if (hit && Date.now() - hit.at < VAULT_TTL_MS)
        return hit;
    const listed = await ctx.mod.run('list', steamId, {}, { quiet: true });
    if (!listed.ok)
        throw new Error(listed.msg || 'the game server did not answer');
    ...
    for (const row of rows) {
        ...
        if (typeof slot !== 'string')
            continue;
        const info = await slotInfo(ctx, steamId, slot);
 ...
 */
export function startWebsite(ctx, log) {
    const web = ctx.config.web;
    if (!web)
        return null;
     ...
     */
    const sameSite = web.allowedOrigins.length > 0 ? 'None' : 'Lax';
    ...
    const corsHeaders = (req) => {
        ...
        if (!originAllowed(origin, web.allowedOrigins))
            return {};
        return {
    ...
    async function sendApp(res, path) {
        if (!appDir) {
    ...
    const route = async (req, res) => {
        ...
        if (path === '/api/status' && req.method === 'GET') {
            ...
            const body = { ok: true, serverName: SERVER, online: statusCache.online };
        ...
        if ((path === '/api/me' || path === '/api/vault') && req.method === 'GET') {
            ...
            if (!who.steamId) {
                sendJson(res, 403, apiError('not_linked', 'This Discord account has no Steam account linked yet. Run /link in Discord.'), cors);
                return;
            }
            if (path === '/api/me') {
            ...
            try {
                ...
                sendJson(res, 503, apiError('server_unreachable', 'The game server did not answer. Your animals are safe, this page just cannot '
    ...
    server.listen(web.port, () => {
        ...
        if (appDir)
            log(`  serving the front end from ${appDir}`);
