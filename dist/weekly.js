...
export const weeklyEnabled = (ctx) => ctx.db.getSetting(ENABLED_KEY) === '1';
export const setWeeklyEnabled = (ctx, on) => ctx.db.setSetting(ENABLED_KEY, on ? '1' : '0');
export const weeklyChannel = (ctx) => ctx.db.getSetting(CHANNEL_KEY) || null;
export const setWeeklyChannel = (ctx, channelId) => ctx.db.setSetting(CHANNEL_KEY, channelId ?? '');
export const weeklySkin = (ctx) => ctx.db.getSetting(SKIN_KEY) || null;
export const setWeeklySkin = (ctx, preset) => ctx.db.setSetting(SKIN_KEY, preset ?? '');
export function buildWeeklyEmbed(week, rows, nameFor, options = {}) {
 ...
 */
export async function closeWeek(ctx, client, week, log) {
    if (ctx.db.getSetting(CLOSED_KEY) === week)
        return null;
    ...
    if (skin) {
        ...
        for (const steamId of ctx.db.skinOwnersFrom(skin, HELD_BY_WEEKLY)) {
            if (keeping.has(steamId))
                continue;
            if (ctx.db.revokeSkin(steamId, skin))
                lost.push(steamId);
        }
    }
...
const nameFor = (ctx) => (steamId) => ctx.db.gameName(steamId) ?? steamId;
/**
 ...
 */
export async function runWeekly(ctx, client, log, at = new Date()) {
    if (!weeklyEnabled(ctx))
        return;
