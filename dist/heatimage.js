 ...
 */
export async function renderRegions(regions, bounds, base, size = SIZE) {
    const image = makeCanvas(size);
    if (base) {
        image.composite((await Jimp.read(base)).resize({ w: size, h: size }), 0, 0);
    }
    ...
    if (bounds) {
        ...
        for (const region of regions) {
            ...
            for (let y = Math.max(0, Math.floor(py - rPixels)); y < Math.min(size, Math.ceil(py + rPixels)); y += 1) {
                for (let x = Math.max(0, Math.floor(px - rPixels)); x < Math.min(size, Math.ceil(px + rPixels)); x += 1) {
                    ...
                    if (!region.active && !edge)
                        continue;
                    // Mixed rather than stamped, so the map underneath still reads.
                    const weight = (edge ? 0xdd : alpha) / 255;
                    const existing = image.getPixelColor(x, y);
                    const mix = (over, under) => Math.round((over * weight) + (under * (1 - weight)));
                    image.setPixelColor((((mix((colour >>> 16) & 0xff, (existing >>> 24) & 0xff) << 24) >>> 0)
                        + (mix((colour >>> 8) & 0xff, (existing >>> 16) & 0xff) << 16)
                        + (mix(colour & 0xff, (existing >>> 8) & 0xff) << 8) + 0xff) >>> 0, x, y);
                }
            }
        }
    }
