...
export const streaksEnabled = (ctx) => ctx.db.getSetting(ENABLED_KEY) !== '0';
export const setStreaksEnabled = (ctx, on) => ctx.db.setSetting(ENABLED_KEY, on ? '1' : '0');
const number = (ctx, key, fallback) => {
    const raw = Number.parseInt(ctx.db.getSetting(key) ?? '', 10);
    return Number.isFinite(raw) && raw >= 0 ? raw : fallback;
};
export const streakStep = (ctx) => number(ctx, STEP_KEY, DEFAULT_STEP);
export const streakCap = (ctx) => number(ctx, CAP_KEY, DEFAULT_CAP);
export const setStreakRewards = (ctx, step, cap) => {
 ...
 */
export function osloDay(at = new Date()) {
 ...
 * stop it.
 */
export function recordPlay(ctx, steamIds, at = new Date()) {
    if (!streaksEnabled(ctx))
        return [];
    ...
    for (const steamId of steamIds) {
        ...
        if (bonus > 0)
            ctx.db.addPoints(steamId, bonus, 0);
        awards.push({
            steamId,
