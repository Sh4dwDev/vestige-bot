...
export class Database {
    ...
    clearBaseline(steamId, species) {
        return Number(species === undefined
    ...
    // ------------------------------------------------------------------ duty --
    #asDuty(row) {
        const num = (key) => typeof row[key] === 'number' ? row[key] : null;
        return {
            ...
            startMethod: String(row['start_method']),
    ...
    startDuty(row) {
        ...
        try {
            this.#db
                .prepare(`INSERT INTO duty_sessions
             (session_id, discord_user_id, steam_id, staff_rank, started_at_utc,
              start_method, status, created_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?)`)
                .run(row.sessionId, row.discordUserId, row.steamId, row.staffRank, row.startedAtUtc, row.startMethod, now, now);
        }
        catch {
    ...
    allActiveDuty() {
        return this.#db
            ...
            .all().map((r) => this.#asDuty(r));
    }
    dutyHistory(discordUserId, limit = 10) {
        return this.#db
     ...
     */
    endDuty(sessionId, endedAtUtc, durationSeconds, reason) {
        return Number(this.#db
            .prepare(`UPDATE duty_sessions
    ...
    setDutyLogMessage(sessionId, channelId, messageId) {
        this.#db
    ...
    createListing(row) {
        return Number(this.#db
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'open', ?)`)
            .run(row.sellerSteam, row.slot, row.species, row.growth, row.female ? 1 : 0, row.prime ? 1 : 0, JSON.stringify(row.mutations), row.price, new Date().toISOString()).lastInsertRowid);
    }
    #asListing(row) {
        let mutations = [];
        try {
            const parsed = JSON.parse(String(row['mutations'] ?? '[]'));
            if (Array.isArray(parsed))
                mutations = parsed.map(String);
        }
        catch {
    ...
    grantSkin(steamId, preset, source) {
        return Number(this.#db
            .prepare(`INSERT INTO owned_skins (steam_id, preset, granted_at, source)
           VALUES (?, ?, ?, ?) ON CONFLICT (steam_id, preset) DO NOTHING`)
            .run(steamId, preset, new Date().toISOString(), source).changes) > 0;
    }
    revokeSkin(steamId, preset) {
        return Number(this.#db
            .prepare('DELETE FROM owned_skins WHERE steam_id = ? AND preset = ?')
            .run(steamId, preset).changes) > 0;
    }
    ownsSkin(steamId, preset) {
        return this.#db
    ...
    ownedSkins(steamId) {
        return this.#db
            .prepare(`SELECT preset, granted_at, source FROM owned_skins
     ...
     * which is which.
     */
    skinOwnersFrom(preset, sourcePrefix) {
        return this.#db
            .prepare(`SELECT steam_id FROM owned_skins
         WHERE preset = ? AND source LIKE ? ORDER BY granted_at ASC`)
            .all(preset, `${sourcePrefix}%`)
    ...
    rememberSpecies(names, origin = 'seen') {
        // A name first met in a curated list can be upgraded once the server
        // actually offers it; never the other way round.
        const stmt = this.#db.prepare(`INSERT INTO known_species (name, first_seen, origin) VALUES (?, ?, ?)
    ...
    setSkin(steamId, species, colours) {
        const merged = { ...(this.skinFor(steamId, species) ?? {}), ...colours };
        this.#db
            .prepare(`INSERT INTO player_skins (steam_id, species, colours, set_at) VALUES (?, ?, ?, ?)
     ...
     */
    expireSkins(olderThanMs) {
        const cutoff = new Date(Date.now() - olderThanMs).toISOString();
        return Number(this.#db.prepare('DELETE FROM player_skins WHERE set_at < ?').run(cutoff).changes);
    }
