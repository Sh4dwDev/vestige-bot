...
export function setHubChannel(ctx, channelId) {
    ctx.db.setSetting(CHANNEL_KEY, channelId ?? '');
    ctx.db.setSetting(HUB_MESSAGE_KEY, '');
}
export function buildHubEmbed() {
    return new EmbedBuilder()
        .setColor(COLORS.info)
        .setTitle(`🦕  ${SERVER} Panel`)
        .setDescription(`Everything ${SERVER} can do for you, in one place. Pick a category — ` +
        ...
        name: '👑  Prime',
        value: 'Run `/prime` in game to see which conditions you still need. Any the '
...
export function hubRows() {
    return [
        new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('hub:archive').setLabel('Archive')
            .setEmoji('🏛️').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('hub:ingame').setLabel('In-game actions')
...
const notLinked = () => new EmbedBuilder()
    ...
    .setDescription('This acts on your live dinosaur, so the bot needs to know which account ' +
    'is yours.\n\nPress **Verify** on the panel — it takes about ten seconds.')
    .setFooter({ text: SIGNATURE });
/** Returns true when the interaction was ours. */
export async function handleHubInteraction(ctx, interaction) {
    ...
    if (id === 'hub:ingame') {
        ...
        await interaction.reply({
            embeds: [new EmbedBuilder()
                    ...
                    .setDescription('These act on the dinosaur you are playing right now, so you must be ' +
                    'on the server.\n\n' +
                    ...
                    .setFooter({ text: SIGNATURE })],
            components: [new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('hub:archive').setLabel('Open archive')
                    .setEmoji('🏛️').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('hub:teleport').setLabel('Travel to a friend')
                    .setEmoji('🧭').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('hub:slay').setLabel('Slay')
    ...
    if (id === 'hub:teleport') {
        ...
        await interaction.reply({
            embeds: [new EmbedBuilder()
                    ...
                    .setDescription('Pick who you want to go to. They have to agree — with a button here ' +
                    'or `!accept` in game — and you move a short while after they do.\n\n' +
                    'You both need to be spawned in.')
                    .setFooter({ text: SIGNATURE })],
            components: [new ActionRowBuilder().addComponents(new UserSelectMenuBuilder().setCustomId('hub:tppick').setPlaceholder('Choose a friend'))],
            flags: MessageFlags.Ephemeral,
        });
        return true;
    }
