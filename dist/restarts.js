...
export const WARNINGS = [60, 30, 15, 5, 1];
 ...
 */
export function nextRestart(now, intervalHours) {
...
export function setRestartsEnabled(ctx, enabled) {
    ctx.db.setSetting(ENABLED_KEY, enabled ? '1' : '0');
}
export function setRestartInterval(ctx, hours) {
    ctx.db.setSetting(HOURS_KEY, String(hours));
}
export function setRestartAnnounce(ctx, channelId, roleId) {
...
export function buildRestartEmbed(minutes, restart) {
    ...
    return new EmbedBuilder()
        ...
        .setDescription(`${SERVER} restarts ${relative}, at ${stamp}.\n\n` +
        (minutes <= 5
 ...
 */
export function startRestartScheduler(ctx, client, log) {
    let cycle = 0;
    const sent = new Set();
    ...
    const tick = async () => {
        ...
        if (warning !== null) {
            for (const w of due)
                sent.add(w);
            const shouldPing = !pinged && DISCORD_WARNINGS.has(warning);
            if (shouldPing)
                pinged = true;
...
async function announce(ctx, client, settings, minutes, restart, log, ping) {
    ...
    if (!settings.channelId || !DISCORD_WARNINGS.has(minutes))
        return;
    try {
 ...
 */
export async function restartNow(ctx, minutes, log) {
    if (!ctx.panel) {
...
async function performRestart(ctx, log) {
...
//# sourceMappingURL=restarts.js.map