...
const MAX_LISTED = 25;
export function setWardrobeChannel(ctx, channelId) {
    ctx.db.setSetting(CHANNEL_KEY, channelId ?? '');
    ctx.db.setSetting(WARDROBE_MESSAGE_KEY, '');
}
export function wardrobeChannel(ctx) {
    return ctx.db.getSetting(CHANNEL_KEY) || null;
}
export function buildWardrobePanel() {
    return new EmbedBuilder()
        .setColor(COLORS.info)
        .setTitle('🎽  Your skins')
        .setDescription(`Every look you have earned on ${SERVER}, in one place.\n\n` +
...
export function wardrobeRows() {
    return [
        new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('wd:mine').setLabel('My skins')
            .setEmoji('🎽').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('wd:reset').setLabel('Reset my colours')
...
export function buildPicker(ctx, steamId) {
    ...
    if (owned.length === 0) {
        return {
            embed: new EmbedBuilder()
                ...
                .setDescription('You have not earned any yet. They are given out at events and by ' +
    ...
    return {
        embed: new EmbedBuilder()
            .setColor(COLORS.info)
            .setTitle('🎽  Your skins')
            .setDescription(`You own **${owned.length}**. Pick one to wear it on the dinosaur you ` +
...
export async function handleWardrobe(ctx, interaction) {
...
async function playingNow(ctx, steamId) {
    const players = await ctx.mod.players().catch(() => []);
    return players.find((p) => p.steam === steamId)?.species ?? null;
}
const notOnADino = () => new EmbedBuilder()
    .setColor(COLORS.warn)
    .setTitle('Not on a dinosaur')
    .setDescription('Colours are painted onto the dinosaur you are playing, so you have to ' +
    'be on one. The spawn screen and admin cam have nothing to paint.')
    .setFooter({ text: SIGNATURE });
async function wear(ctx, interaction, steamId, name) {
...
async function resetColours(ctx, interaction, steamId) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    const species = await playingNow(ctx, steamId);
    if (!species) {
        await interaction.editReply({ embeds: [notOnADino()] });
