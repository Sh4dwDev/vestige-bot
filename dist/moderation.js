...
const embed = (color, title, description) => new EmbedBuilder().setColor(color).setTitle(title).setDescription(description)
    .setFooter({ text: SIGNATURE }).setTimestamp();
export function setModLogChannel(ctx, channelId) {
    ctx.db.setSetting(LOG_KEY, channelId ?? '');
}
export function modLogChannel(ctx) {
    return ctx.db.getSetting(LOG_KEY) || null;
}
/** Never throws: an audit line failing must not fail the action itself. */
async function audit(ctx, i, title, detail) {
 ...
 */
async function steamFor(ctx, i, optionName = 'user') {
    const user = i.options.getUser(optionName, true);
    const link = ctx.db.linkFor(user.id);
    if (!link) {
        await i.editReply({
            embeds: [embed(COLORS.warn, 'Not linked', `${user} has not linked a Steam account, so the server does not know ` +
...
// -------------------------------------------------------------- moderation --
export async function handleModeration(ctx, i, action) {
    if (action === 'log') {
        const channel = i.options.getChannel('channel', true);
        setModLogChannel(ctx, channel.id);
        await i.reply({
            embeds: [embed(COLORS.good, 'Staff log set', `Kicks, bans and whitelist changes will be posted in <#${channel.id}>, ` +
                    'with who did them.')],
    ...
    try {
        ...
        if (action === 'ban') {
            ...
            await i.editReply({
                embeds: [embed(COLORS.bad, hours > 0 ? `Banned for ${hours}h` : 'Banned permanently', `${target.label}\n\nReason: ${reason}` +
                        (reply ? `\n\nServer said: \`${reply}\`` : '') +
                        '\n\nThis bans the Steam account on the game server. It does not touch ' +
                        'their Discord account.')],
            });
            await audit(ctx, i, hours > 0 ? `Player banned (${hours}h)` : 'Player banned permanently', `${target.label}\nReason: ${reason}`);
            return;
        }
        if (action === 'whitelist') {
            ...
            if (mode === 'toggle') {
                ...
                await i.editReply({
                    embeds: [embed(COLORS.warn, 'Whitelist toggled', `Server said: \`${reply || '(no reply)'}\`\n\n` +
            ...
            await i.editReply({
                embeds: [embed(COLORS.good, mode === 'add' ? 'Added to the whitelist' : 'Removed', `${target.label}` + (reply ? `\n\nServer said: \`${reply}\`` : ''))],
            });
        ...
        if (action === 'globalchat') {
            ...
            await i.editReply({
                embeds: [embed(COLORS.warn, 'Global chat toggled', `Server said: \`${reply || '(no reply)'}\`\n\n` +
        ...
        if (action === 'say') {
            ...
            await i.editReply({
                embeds: [embed(COLORS.good, 'Announced', `Sent to everyone in game:\n\n> ${message}`)],
            });
        ...
        if (action === 'tell') {
            ...
            await i.editReply({
                embeds: [shown
                        ...
                        : embed(COLORS.warn, 'Could not show it', `${target.label} is not on a dinosaur right now, so there is nothing ` +
...
export async function handleInGame(ctx, i, action) {
    ...
    if (!me && action !== 'heal') {
        await i.editReply({
            embeds: [embed(COLORS.warn, 'Link first', 'This moves someone relative to **you**, so the bot needs to know which ' +
    ...
    try {
        ...
        if (action === 'heal') {
            ...
            await i.editReply({
                embeds: [result.ok
                        ? embed(COLORS.good, 'Patched up', `${target.label} is back to full health, food, water and stamina.\n\n` +
        ...
        await i.editReply({
            embeds: [result.ok
                    ? embed(COLORS.good, action === 'bring' ? 'Brought them to you' : 'Moved you to them', `${result.msg}\n\n${target.label}`)
                    : embed(COLORS.bad, 'Could not move anyone', result.msg)],
        });
        await audit(ctx, i, action === 'bring' ? 'Player brought to admin' : 'Admin moved to player', target.label);
    }
    catch (err) {
        await i.editReply({
            embeds: [embed(COLORS.bad, 'Could not do that', describeError(err))],
