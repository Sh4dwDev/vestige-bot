 ...
 */
export function marketFee(ctx) {
    const raw = Number.parseInt(ctx.db.getSetting(FEE_KEY) || '0', 10);
    return Number.isFinite(raw) && raw >= 0 && raw <= 50 ? raw : 0;
}
export const setMarketFee = (ctx, percent) => ctx.db.setSetting(FEE_KEY, String(Math.max(0, Math.min(50, Math.round(percent)))));
/** What the seller actually receives, after any cut. */
export const sellerTakes = (price, feePercent) => Math.max(0, price - Math.floor((price * feePercent) / 100));
/** Reads what is actually in one of somebody's slots. */
export async function slotInfo(ctx, steamId, slot) {
    try {
...
export async function slotsUsed(ctx, steamId) {
    try {
 ...
 */
export async function listForSale(ctx, sellerSteam, slot, price) {
    ...
    if (!moved.ok)
        return { ok: false, reason: moved.msg };
 ...
 */
export async function buyListing(ctx, id, buyerSteam) {
    const listing = ctx.db.listing(id);
    if (!listing)
        return { ok: false, reason: 'That listing no longer exists.' };
...
export async function cancelListing(ctx, id, bySteam) {
    const listing = ctx.db.listing(id);
    if (!listing)
        return { ok: false, reason: 'That listing no longer exists.' };
...
// ------------------------------------------------------------------ embeds --
const percent = (growth) => `${Math.round(growth * 100)}%`;
/** One line describing what is on offer, for menus and summaries. */
export const describeListing = (listing) => `${listing.species} · ${percent(listing.growth)} · ${listing.female ? 'F' : 'M'}`
    + (listing.prime ? ' · Prime' : '')
    + (listing.mutations.length > 0 ? ` · ${listing.mutations.length} mut` : '')
    + ` — ${listing.price} pts`;
export function buildListingEmbed(listing, nameFor, fee = 0) {
    ...
    const embed = new EmbedBuilder()
        ...
        .addFields({
        name: 'What it is',
        value: `Growth **${percent(listing.growth)}**\n`
...
export const listingRows = (listing) => listing.status === 'open' || listing.status === 'pending'
    ? [
        new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`mk:buy:${listing.id}`).setLabel('Buy')
            .setEmoji('💰').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId(`mk:cancel:${listing.id}`).setLabel('Take it down')
...
export const marketRows = () => [
    new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('mk:sell').setLabel('Sell')
        .setEmoji('📦').setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId('mk:browse').setLabel('Browse')
        .setEmoji('🔎').setStyle(ButtonStyle.Primary), new ButtonBuilder().setCustomId('mk:mine').setLabel('My listings')
        .setEmoji('🧾').setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId('hub:verify').setLabel('Verify')
...
export function buildSellPicker(slots) {
    if (slots.length === 0) {
        return {
...
export function buildBrowseEmbed(listings, nameFor) {
...
const steamNameFor = (ctx) => (steamId) => ctx.db.gameName(steamId) ?? steamId;
export function buildPriceModal(slot) {
 ...
 */
export async function handleMarket(ctx, interaction) {
...
async function openSellPicker(ctx, interaction, steamId) {
...
async function submitListing(ctx, interaction, steamId, slot) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    ...
    await interaction.editReply({
        embeds: [new EmbedBuilder().setColor(COLORS.good).setTitle('Listed')
                .setDescription(`Your **${result.listing.species}** is on the market for **${price}** points.`
                + (fee > 0
...
async function completeBuy(ctx, interaction, buyerSteam, id) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    ...
    await interaction.editReply({
        embeds: [new EmbedBuilder().setColor(COLORS.good).setTitle('Bought')
                .setDescription(`The **${result.listing.species}** is in your archive. `
    ...
    await user?.send({
        embeds: [new EmbedBuilder().setColor(COLORS.good).setTitle('Your dinosaur sold')
                .setDescription(`Your **${result.listing.species}** sold for **${result.paid}** points`
...
async function completeCancel(ctx, interaction, bySteam, id) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
