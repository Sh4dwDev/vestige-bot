...
export const auditChannel = (ctx) => ctx.db.getSetting(CHANNEL_KEY) || null;
export const setAuditChannel = (ctx, channelId) => ctx.db.setSetting(CHANNEL_KEY, channelId ?? '');
 ...
 */
export function describeOptions(i) {
    const parts = [];
            ...
            for (const child of children) {
                ...
                if (nested.length > 0) {
                    for (const leaf of nested)
                        parts.push(`${leaf.name}: ${String(leaf.value ?? '')}`);
                }
...
export function buildAuditEmbed(entry) {
 ...
 */
export async function writeAudit(ctx, client, entry) {
