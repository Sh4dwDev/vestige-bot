...
export function osloTime(at) {
    ...
    const day = DAYS.indexOf(get('weekday').toLowerCase().slice(0, 3));
    return {
 ...
 * exception.
 */
export function isWeekend(at, window) {
...
export const weekendActive = (ctx, at = new Date()) => weekendBonus(ctx) > 0 && isWeekend(at, weekendWindow(ctx));
 ...
 */
function occurrence(day, hour, from) {
 ...
 * hours it looks like.
 */
export function windowInstance(window, at = new Date()) {
 ...
 */
export const describeWindow = (window, at = new Date()) => {
