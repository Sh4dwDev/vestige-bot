...
export function setSpeciesChannel(ctx: Ctx, channelId: string | null): void {
  ctx.db.setSetting(CHANNEL_KEY, channelId ?? '');
}

export function speciesChannel(ctx: Ctx): string | null {
 ...
 */
export async function checkSpeciesLocks(
  ctx: Ctx,
  client: Client,
  players: PlayerRow[],
  log: (m: string) => void,
