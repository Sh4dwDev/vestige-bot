...
export const commandData = [
  ...
  new SlashCommandBuilder()
    ...
    .addSubcommand((c) =>
      c.setName('setup').setDescription('Announcement channel, and optionally the map')
        .addChannelOption((o) =>
          o.setName('channel').setDescription('Where events are announced')
            .addChannelTypes(ChannelType.GuildText).setRequired(true))
        .addChannelOption((o) =>
          o.setName('map').setDescription('Where the region map is kept')
            .addChannelTypes(ChannelType.GuildText))
        .addRoleOption((o) =>
          o.setName('role').setDescription('Pinged on start. Off unless set'))
        .addBooleanOption((o) =>
          o.setName('auto').setDescription('Run events automatically. Default on')))
    .addSubcommand((c) =>
      c.setName('start').setDescription('Start one now')
        .addStringOption((o) =>
          o.setName('region').setDescription('Which one. Random if left out')
            .setAutocomplete(true))
        .addIntegerOption((o) =>
          o.setName('minutes').setDescription('How long. Default 45')
            .setMinValue(1).setMaxValue(240))
        .addIntegerOption((o) =>
          o.setName('reward').setDescription('Points each. Default 300')
            .setMinValue(0).setMaxValue(100_000))
        .addIntegerOption((o) =>
          o.setName('required').setDescription('Active minutes needed. Default 15')
            .setMinValue(1).setMaxValue(240))
        .addBooleanOption((o) =>
      ...
      c.setName('move').setDescription('Put a region centre where you are standing')
        .addStringOption((o) =>
          o.setName('region').setDescription('Which one').setRequired(true)
            .setAutocomplete(true))
        .addIntegerOption((o) =>
          o.setName('radius').setDescription('Radius in HUD units. Default keeps it')
            .setMinValue(5).setMaxValue(1000))),
...
const hudUnits = (world: number): number => Math.round(world / 1000);
...
async function handleRegions(
  ...
  if (action === 'setup') {
    ...
    if (map) setRegionSetting(ctx, 'mapChannel', map.id);
    if (role) setRegionSetting(ctx, 'roleMention', role.id);
  ...
  if (action === 'status') {
    ...
    await i.editReply({
      embeds: [embed(COLORS.good, `🔥  ${running.regionName}`,
...
It joins the rotation straight away. Adding the same `
  ...
  if (action === 'move') {
    ...
    if (!me || me.x === undefined || me.y === undefined) {
      await i.editReply({
        embeds: [embed(COLORS.warn, 'Cannot see you',
          'Stand in the middle of the area in game and run this again.')],
      });
      return;
    }
...
`
  ...
  if (action === 'stop') {
    ...
    await i.editReply({
      embeds: [embed(COLORS.good, 'Stopped',
        `**${running.regionName}** ended early. `
  ...
  const started = startEvent(ctx, {
    ...(i.options.getString('region') ? { regionId: i.options.getString('region') as string } : {}),
    ...(i.options.getInteger('minutes') !== null
      ? { minutes: i.options.getInteger('minutes') as number } : {}),
    ...(i.options.getInteger('reward') !== null
      ? { reward: i.options.getInteger('reward') as number } : {}),
    ...(i.options.getInteger('required') !== null
      ? { requiredMinutes: i.options.getInteger('required') as number } : {}),
