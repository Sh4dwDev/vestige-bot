  ...
  new SlashCommandBuilder()
    ...
    .addSubcommandGroup((g) =>
      g.setName('ai').setDescription('Wildlife')
        ...
        .addSubcommand((s) =>
          s.setName('auto').setDescription('Wildlife that spawns near players on its own')
            .addBooleanOption((o) =>
              o.setName('on').setDescription('Keep the island populated')
                .setRequired(true))
            .addIntegerOption((o) =>
              o.setName('per_player').setDescription('How many to keep near each player (default 6)')
                .setMinValue(0).setMaxValue(20))
            .addIntegerOption((o) =>
              o.setName('cap').setDescription('Server-wide ceiling (default 60)')
                .setMinValue(0).setMaxValue(200)))
        .addSubcommand((s) =>
          s.setName('brain').setDescription('Attacking brain, or the one that escapes a pounce')
            .addStringOption((o) =>
              o.setName('species').setDescription('Which animal')
                .setAutocomplete(true).setRequired(true))
            .addStringOption((o) =>
              o.setName('brain').setDescription('attack fights back, evade shakes off a pounce')
                .addChoices(
                  { name: 'attack — fights back, but cannot shake off a pounce', value: 'attack' },
                  { name: 'evade — escapes a pounce, but will not attack', value: 'evade' })
                .setRequired(true)))
        .addSubcommand((s) =>
...
async function handleSkin(
  ...
  if (action === 'reset') {
    ...
    await i.reply({
      embeds: [cleared > 0
        ? embed(COLORS.good, 'Colours forgotten',
...
` +
  ...
  if (action === 'auto') {
    ...
    try {
      ...
      await i.editReply({
        embeds: [on
          ? embed(COLORS.good, 'Ambient wildlife on',
            `${result.msg}.\n\n` +
          ...
          : embed(COLORS.good, 'Ambient wildlife off',
            'Nothing new will spawn. What is already out there stays until it ' +
  ...
  if (action === 'brain') {
    const species = i.options.getString('species', true).trim();
    const brain = i.options.getString('brain', true) as 'attack' | 'evade';
    await i.deferReply({ flags: MessageFlags.Ephemeral });
