...
export function earlyMinutes(ctx: Ctx): number {
  const raw = Number.parseInt(ctx.db.getSetting(MINUTES_KEY) ?? '', 10);
  return Number.isFinite(raw) && raw >= 0 ? raw : DEFAULT_MINUTES;
}

export const setEarlyMinutes = (ctx: Ctx, minutes: number): void =>
  ctx.db.setSetting(MINUTES_KEY, String(Math.max(0, Math.round(minutes))));
 ...
 */
export function minutesPlayed(ctx: Ctx, discordId: string): number | null {
...
export const hasPlayedEnough = (minutes: number | null, required: number): boolean =>
  minutes !== null && minutes >= required;
...
export async function backfillEarlyRole(
  ...
  const roleId = earlyRole(ctx);
  if (!roleId) return { given: 0, already: 0, skipped: 0, full: false, unqualified: 0 };
  ...
  const out: Backfill = { given: 0, already: 0, skipped: 0, full: false, unqualified: 0 };
  ...
  for (const member of ordered) {
    ...
    else if (result === 'unqualified') out.unqualified += 1;
    else out.skipped += 1;
  }
