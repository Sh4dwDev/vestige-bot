...
export function nestingSettings(ctx: Ctx): NestingSettings {
  return {
...
export const setNestingEnabled = (ctx: Ctx, on: boolean): void =>
  ctx.db.setSetting(ENABLED, on ? '1' : '0');

export const setNestingPoints = (ctx: Ctx, points: number): void =>
  ctx.db.setSetting(PARENT_KEY, String(Math.max(0, Math.round(points))));

export const setNestingRadius = (ctx: Ctx, hud: number): void =>
  ctx.db.setSetting(RADIUS_KEY, String(Math.max(1, Math.round(hud))));

export const setNestingCondition = (ctx: Ctx, index: number): void =>
  ctx.db.setSetting(CONDITION_KEY, String(Math.max(1, Math.min(10, Math.round(index)))));
...
export function parentsOf(
  ...
  if (hatchling.x === undefined || hatchling.y === undefined) return [];
  ...
  return players
    ...
    .map((p) => ({
      steam: p.steam as string,
      away: Math.hypot(
        (p.x as number) - (hatchling.x as number),
        (p.y as number) - (hatchling.y as number),
      ),
    }))
...
export function hatchlingCandidates(
  players: PlayerRow[],
  growth: number,
...
export function grownUp(
  ...
  const small = new Set(
    players.filter((p) => p.steam && p.growth <= growth).map((p) => p.steam as string),
  );
  return [...checked].filter((steam) => !small.has(steam));
}

/** The in-game line. ASCII only: the mod drops anything else silently. */
export const nestAnnounce = (species: string, parents: number, points: number): string =>
  `NEST: a ${species} nest hatched. ${parents} parent${parents === 1 ? '' : 's'} `
  + `paid ${points} points each.`;

/** What the parent sees on their own screen. */
export const parentNotice = (species: string, points: number): string =>
  `+${points} points - your ${species} nest hatched.`;
...
export const forgetChecked = (): void => checked.clear();
...
export async function runNesting(
  ...
  for (const steam of grownUp(players, settings.growth, checked)) checked.delete(steam);

  const out: NestOutcome[] = [];

  for (const hatchling of hatchlingCandidates(players, settings.growth, checked)) {
    const steam = hatchling.steam as string;
