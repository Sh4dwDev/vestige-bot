...
export class AdminStore {
  ...
  constructor(
    ...
    private readonly gameIniPath: string,
  ...
  async readIni(): Promise<string> {
    const raw = await this.#withClient((client) => client.get(this.gameIniPath));
    if (!Buffer.isBuffer(raw)) throw new Error('unexpected Game.ini response');
  ...
  static parseAdmins(ini: string): string[] {
    ...
    for (const line of ini.split(/\r?\n/)) {
      ...
      const value = match?.[1];
      if (value && /^7656119\d{10}$/.test(value)) found.push(value);
    }
  ...
  static replaceAdmins(ini: string, steamIds: string[]): string {
    ...
    for (const line of lines) {
      ...
      if (!inserted && line.trim().toLowerCase() === `[${SECTION}]`) {
        for (const id of steamIds) out.push(`${ADMIN_KEY}=${id}`);
        inserted = true;
      }
    }
  ...
  async adoptExisting(): Promise<number> {
    if (this.db.getSetting('admins_seeded') !== null) return 0;

    const existing = AdminStore.parseAdmins(await this.readIni());
    for (const steamId of existing) this.db.addGameAdmin(steamId, 'existing config');
