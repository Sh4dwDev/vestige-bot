...
export function restartAlertRole(ctx: Ctx): string | null {
  return ctx.db.getSetting(KEY) || null;
}

export function setRestartAlertRole(ctx: Ctx, roleId: string | null): void {
 ...
 */
export async function handleRestartRoleButton(
  ctx: Ctx,
  interaction: ButtonInteraction,
  log: (m: string) => void,
): Promise<boolean> {
  if (interaction.customId !== RESTART_ROLE_BUTTON) return false;

  const roleId = restartAlertRole(ctx);
  if (!roleId) {
    await reply(interaction,
      'Restart alerts are not set up yet — an admin needs to choose the role '
  ...
  try {
    ...
    log(`restartrole: could not ${had ? 'remove' : 'add'} ${roleId} for ${
      interaction.user.id}: ${reason}`);
    await reply(interaction,
      'Discord would not let me change your roles. The bot needs **Manage '
