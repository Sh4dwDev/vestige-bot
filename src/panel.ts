 ...
 */
const relative = (unix: number): string =>
  unix > 1_000_000_000 ? `<t:${Math.floor(unix)}:R>` : 'at an unknown time';
...
export async function buildPanel(
  ...
  if (error) {
    ...
    embed.setDescription(
      'Nothing of yours is kept here yet.\n\n' +
  ...
  if (options.notice) {
    ...
    embed.setDescription(
      `${marks[options.notice.tone]} ${options.notice.text}` +
      (embed.data.description ? `\n\n${embed.data.description}` : ''),
    );
    if (options.notice.tone === 'good') embed.setColor(COLORS.good);
    if (options.notice.tone === 'bad') embed.setColor(COLORS.bad);
  }
  ...
  rows.push(
    new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder()
        ...
        .setDisabled(slots.length >= MAX_SLOTS),
      new ButtonBuilder()
      ...
      new ButtonBuilder()
        ...
        .setDisabled(selected === null),
      new ButtonBuilder().setCustomId('st:refresh').setLabel('Refresh').setStyle(ButtonStyle.Secondary),
    ),
  );
...
type Editable = { editReply: (options: InteractionUpdateOptions) => Promise<unknown> };
...
function confirmRows(action: string, slot: string): ActionRowBuilder<ButtonBuilder>[] {
  return [
    new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder()
        ...
        .setStyle(ButtonStyle.Danger),
      new ButtonBuilder().setCustomId('st:refresh').setLabel('Cancel').setStyle(ButtonStyle.Secondary),
    ),
  ];
}
  ...
  try {
    ...
    if (action === 'delete') {
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `\`${slot}\` will be struck from the archive for good. ` +
