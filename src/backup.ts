...
export function startBackupScheduler(ctx: Ctx, cfg: BackupConfig, log: (m: string) => void): void {
  const tick = async (): Promise<void> => {
    if (Date.now() - lastBackup(ctx) < backupEveryHours(ctx) * 3_600_000) return;
    try {
