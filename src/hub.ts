  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      },
      {
        name: '🔔  Restart alerts',
        value:
          'Give yourself the ping for scheduled restarts, or take it back. '
...
export async function handleHubInteraction(
  ...
  if (id === 'hub:profile') {
    ...
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
    await interaction.editReply({
      embeds: [buildProfileEmbed(await gatherProfile(
        ctx, interaction.user.id, link.steamId, interaction.user.displayAvatarURL()))],
    });
    return true;
  }
