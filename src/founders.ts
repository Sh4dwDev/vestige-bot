 ...
 */
export const FOUNDER_SKINS: FounderSkin[] = [
  {
    id: 'firstlight',
    name: 'First Light',
    emoji: '🌅',
...
export const skinById = (id: string): FounderSkin | undefined =>
  FOUNDER_SKINS.find((s) => s.id === id);

// ---------------------------------------------------------------- settings --

export function setFounderChannel(ctx: Ctx, channelId: string | null): void {
  ctx.db.setSetting(CHANNEL_KEY, channelId ?? '');
  ctx.db.setSetting(FOUNDER_MESSAGE_KEY, '');
}

export function founderChannel(ctx: Ctx): string | null {
  return ctx.db.getSetting(CHANNEL_KEY) || null;
}

export function founderLimit(ctx: Ctx): number {
  const raw = Number.parseInt(ctx.db.getSetting(LIMIT_KEY) ?? '', 10);
  return Number.isFinite(raw) && raw >= 0 ? raw : DEFAULT_LIMIT;
}

export function setFounderLimit(ctx: Ctx, limit: number): void {
...
export async function handleFounderInteraction(
  ctx: Ctx,
...
async function applyFounderSkin(
  ctx: Ctx,
  interaction: ButtonInteraction,
  steamId: string,
  ...
  try {
    ...
    const playing = (await ctx.mod.players().catch(() => []))
      .find((p) => p.steam === steamId)?.species;
    if (playing) await captureBaseline(ctx, steamId, playing);
