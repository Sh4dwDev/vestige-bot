...
import { completePurchase, type Ctx } from './commands.js';
...
export function setShopPanelChannel(ctx: Ctx, channelId: string | null): void {
  ctx.db.setSetting(CHANNEL_KEY, channelId ?? '');
  ctx.db.setSetting(SHOP_PANEL_MESSAGE_KEY, '');
}

export function buildShopPanel(ctx: Ctx): EmbedBuilder {
  return new EmbedBuilder()
    .setColor(COLORS.info)
    .setTitle(`🛒  The ${SERVER} shop`)
    .setDescription(
      'Spend the points you earn by playing on a **fully grown** dinosaur, ' +
    ...
    .addFields(
      ...
      },
      {
        ...
        value:
          `It uses one of your ${MAX_SLOTS} vaults, and purchases are **not ` +
...
export function shopPanelRows(): ActionRowBuilder<ButtonBuilder>[] {
  return [
    new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId('shop:browse').setLabel('Browse')
        .setEmoji('📋').setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId('shop:start').setLabel('Buy')
...
function basket(ctx: Ctx, discordId: string, balance: number): {
  ...
  const option = (name: string): { label: string; value: string; description?: string } => {
