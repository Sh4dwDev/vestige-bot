...
export type EndReason =
  | 'Staff went off duty'
  | 'Forced off duty by senior staff'
...
const get = (ctx: Ctx, key: string): string | null => ctx.db.getSetting(key) || null;

export const staffRole = (ctx: Ctx): string | null => get(ctx, KEYS.staffRole);
export const onDutyRole = (ctx: Ctx): string | null => get(ctx, KEYS.onDutyRole);
export const seniorRole = (ctx: Ctx): string | null => get(ctx, KEYS.seniorRole);
export const dutyLogChannel = (ctx: Ctx): string | null => get(ctx, KEYS.logChannel);
export const dutyPanelChannel = (ctx: Ctx): string | null => get(ctx, KEYS.panelChannel);
export const DUTY_PANEL_MESSAGE_KEY = KEYS.panelMessage;

export function maxHours(ctx: Ctx): number {
  const raw = Number.parseFloat(ctx.db.getSetting(KEYS.maxHours) ?? '');
  return Number.isFinite(raw) && raw > 0 ? raw : DEFAULT_MAX_HOURS;
}

export const setDutySetting = (ctx: Ctx, key: keyof typeof KEYS, value: string | null): void =>
 ...
 */
export function nextSessionId(ctx: Ctx, now: Date): string {
...
export const durationBetween = (startUtc: string, endUtc: string): number =>
  Math.max(0, Math.round((Date.parse(endUtc) - Date.parse(startUtc)) / 1000));

/** Discord renders this in each viewer's own timezone, which beats picking one. */
export const stamp = (iso: string): string =>
  `<t:${Math.floor(Date.parse(iso) / 1000)}:F>`;
 ...
 */
export function dutyRanks(ctx: Ctx): DutyRank[] {
  const stored = storedRanks(ctx);
  if (stored.length > 0) return stored;

  const legacy: DutyRank[] = [];
  const senior = seniorRole(ctx);
  const staff = staffRole(ctx);
  if (senior) legacy.push({ roleId: senior, label: 'Head Admin', level: 90, canForceOff: true });
  if (staff) legacy.push({ roleId: staff, label: 'Staff', level: 10, canForceOff: false });
  return legacy;
}
 ...
 */
export function storedRanks(ctx: Ctx): DutyRank[] {
...
export const setDutyRanks = (ctx: Ctx, ranks: DutyRank[]): void =>
  ctx.db.setSetting(KEYS.ranks, JSON.stringify(ranks));

/** Adds or replaces one rank, keeping the list sorted by seniority. */
export function upsertDutyRank(ctx: Ctx, rank: DutyRank): DutyRank[] {
...
export function removeDutyRank(ctx: Ctx, roleId: string): DutyRank[] {
...
export function buildDutyPanel(): EmbedBuilder {
  return new EmbedBuilder()
    .setColor(COLORS.off)
    .setTitle('🛡️  Vestige Staff Duty')
    .setDescription(
      'Use this panel whenever you begin or finish an in-game moderation '
...
export const dutyPanelRows = (): ActionRowBuilder<ButtonBuilder>[] => [
  new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId('duty:on').setLabel('Go On Duty')
      .setEmoji('🟢').setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId('duty:off').setLabel('Go Off Duty')
      .setEmoji('⚪').setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId('duty:active').setLabel('Active Staff')
      .setEmoji('📋').setStyle(ButtonStyle.Primary),
  ),
];

export function buildStartLog(session: DutySession, username: string): EmbedBuilder {
  return new EmbedBuilder()
    .setColor(COLORS.on)
    .setTitle('🟢  Staff on duty')
    .setDescription(`<@${session.discordUserId}> · **${username}**`)
    .addFields(
      { name: 'Discord ID', value: session.discordUserId, inline: true },
      { name: 'Steam ID', value: session.steamId || 'not linked', inline: true },
      { name: 'Rank', value: session.staffRank, inline: true },
      { name: 'Session', value: session.sessionId, inline: true },
      { name: 'Started', value: stamp(session.startedAtUtc), inline: true },
      { name: 'Started by', value: startedByLabel(session.startMethod), inline: true },
    )
    .setFooter({ text: `${SERVER} · Active` })
    .setTimestamp(new Date(session.startedAtUtc));
}

const startedByLabel = (method: StartMethod): string =>
  method === 'panel' ? 'Duty panel' : method === 'command' ? 'Slash command' : 'Override';

export function buildEndLog(session: DutySession, username: string): EmbedBuilder {
...
export async function goOnDuty(
  ctx: Ctx,
  member: { id: string; roles: { cache: { has: (id: string) => boolean }; add: (id: string) => Promise<unknown> } },
  rank: string,
  method: StartMethod,
  log: (m: string) => void,
  ...
  if (!session) return { ok: false, reason: 'You are already on duty.' };
  ...
  if (roleId) {
    try {
      ...
      log(`duty: could not add the on-duty role for ${member.id}: ${
 ...
 */
export async function goOffDuty(
  ctx: Ctx,
  member: { id: string; roles: { remove: (id: string) => Promise<unknown> } } | null,
  discordUserId: string,
  reason: EndReason | string,
  log: (m: string) => void,
): Promise<EndOutcome> {
  const open = ctx.db.activeDuty(discordUserId);
  if (!open) return { ok: false, reason: 'You are not on duty.' };
  ...
  if (roleId && member) {
    try {
      ...
      log(`duty: could not remove the on-duty role for ${discordUserId}: ${
        err instanceof Error ? err.message : String(err)}`);
    }
  }
...
export function ranksFor(ctx: Ctx): Array<{ roleId: string; label: string }> {
 ...
 */
export const isStaff = (ctx: Ctx, roleIds: readonly string[]): boolean =>
  dutyRanks(ctx).some((r) => roleIds.includes(r.roleId));

/** Whether any rank they hold is allowed to act on other people's sessions. */
export const isSenior = (ctx: Ctx, roleIds: readonly string[]): boolean =>
  dutyRanks(ctx).some((r) => r.canForceOff && roleIds.includes(r.roleId));
 ...
 */
export async function postStartLog(
  ctx: Ctx,
  client: Client,
  session: DutySession,
  username: string,
  ...
  try {
    ...
    log(`duty: start log failed for ${session.sessionId}: ${
      err instanceof Error ? err.message : String(err)}`);
  }
}
 ...
 */
export async function postEndLog(
  ctx: Ctx,
  client: Client,
  session: DutySession,
  username: string,
  ...
  try {
    ...
    log(`duty: end log failed for ${session.sessionId}: ${
      err instanceof Error ? err.message : String(err)}`);
  }
}
...
const deny = (title: string, body: string): EmbedBuilder =>
  new EmbedBuilder().setColor(COLORS.bad).setTitle(title).setDescription(body)
    .setFooter({ text: SIGNATURE });

/** Returns true when the interaction was ours. */
export async function handleDuty(
  ctx: Ctx,
  interaction: ButtonInteraction,
  log: (m: string) => void,
): Promise<boolean> {
  const id = interaction.customId;
  if (!id.startsWith('duty:')) return false;
  ...
  if (id === 'duty:on') {
    const result = await goOnDuty(
      ctx, member, rankOf(roleIds, ranksFor(ctx)), 'panel', log);
    ...
    await interaction.editReply({
      embeds: [new EmbedBuilder().setColor(COLORS.on).setTitle('🟢  On duty')
        .setDescription(
          `Session **${result.session.sessionId}** started ${stamp(result.session.startedAtUtc)}.`
  ...
  if (id === 'duty:off') {
    const result = await goOffDuty(
      ctx, member, interaction.user.id, 'Staff went off duty', log);
 ...
 */
export async function reconcileDuty(
  ctx: Ctx,
  client: Client,
  log: (m: string) => void,
  { onStartup = false }: { onStartup?: boolean } = {},
): Promise<number> {
  ...
  for (const open of ctx.db.allActiveDuty()) {
    ...
    const reason: EndReason | null = onStartup
      ? 'Bot restarted'
      : age >= limitMs
        ? 'Maximum session duration reached'
        : null;
    ...
    await postEndLog(ctx, client, result.session,
      member?.user?.username ?? open.discordUserId, log);
  ...
  if (onStartup) await stripOrphanRoles(ctx, client, log);

  return closed;
}

async function stripOrphanRoles(
  ctx: Ctx,
  client: Client,
  log: (m: string) => void,
): Promise<void> {
  ...
  for (const guild of client.guilds.cache.values()) {
    ...
    for (const member of role.members.values()) {
      if (ctx.db.activeDuty(member.id)) continue;
