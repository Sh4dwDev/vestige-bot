...
export const PER_HUNDRED: Record<string, number> = {
 ...
 */
export function planCaps(ctx: Ctx, maxPlayers: number, available: string[]): PlannedCap[] {
...
export function applyCaps(ctx: Ctx, planned: PlannedCap[]): void {
  for (const entry of planned) ctx.db.setSpeciesCap(entry.species, entry.cap);
}
