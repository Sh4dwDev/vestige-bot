...
const PAD = { left: 44, right: 16, top: 34, bottom: 26 } as const;
...
type Canvas = InstanceType<typeof Jimp>;

/** Alpha blend, so the area fill lets the grid show through. */
function blend(image: Canvas, x: number, y: number, colour: Rgba, alpha: number): void {
  if (x < 0 || y < 0 || x >= WIDTH || y >= HEIGHT || alpha <= 0) return;

  const under = rgba(image.getPixelColor(x, y));
  const a = Math.min(1, alpha);
  const mix = (over: number, beneath: number): number =>
    Math.round((over * a) + (beneath * (1 - a)));

  image.setPixelColor(
    ((mix(colour.r, under.r) << 24)
...
function hLine(image: Canvas, x0: number, x1: number, y: number, colour: Rgba, alpha = 1): void {
  for (let x = x0; x <= x1; x += 1) blend(image, x, y, colour, alpha);
}

function vLine(image: Canvas, x: number, y0: number, y1: number, colour: Rgba, alpha = 1): void {
  for (let y = y0; y <= y1; y += 1) blend(image, x, y, colour, alpha);
}
 ...
 */
export async function renderChart(buckets: Bucket[], labels: ChartLabels): Promise<Buffer> {
  ...
  const xFor = (index: number): number => buckets.length <= 1
    ? PLOT.x0 + (plotWidth / 2)
    : PLOT.x0 + ((index / (buckets.length - 1)) * plotWidth);
  const yFor = (value: number): number => PLOT.y1 - ((value / top) * plotHeight);
  ...
  for (let index = 0; index < points.length; index += 1) {
    ...
    for (let dx = -1; dx <= 1; dx += 1) {
      for (let dy = -1; dy <= 1; dy += 1) blend(image, here.x + dx, here.y + dy, line, 1);
    }
  }

  // The peak, marked where it happened.
  const peakPoint = points.reduce<{ x: number; y: number; value: number } | null>(
    (best, p) => (p && (!best || p.value > best.value) ? p : best), null);
