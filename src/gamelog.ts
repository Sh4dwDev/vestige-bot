...
export const gameLogEnabled = (ctx: Ctx): boolean =>
  ctx.db.getSetting(ENABLED_KEY) === '1';

export const setGameLogEnabled = (ctx: Ctx, on: boolean): void =>
  ctx.db.setSetting(ENABLED_KEY, on ? '1' : '0');

const offset = (ctx: Ctx): number => {
  const raw = Number.parseInt(ctx.db.getSetting(OFFSET_KEY) ?? '', 10);
  return Number.isFinite(raw) && raw >= 0 ? raw : 0;
};

const setOffset = (ctx: Ctx, at: number): void =>
  ctx.db.setSetting(OFFSET_KEY, String(at));
 ...
 */
export function parseCommandLines(chunk: string): CommandLine[] {
...
export function buildGameLogEmbed(line: CommandLine): EmbedBuilder {
  ...
  if (line.args) embed.addFields({ name: 'With', value: line.args.slice(0, 1024) });
 ...
 * one.
 */
export async function runGameLog(
  ctx: Ctx,
  client: Client,
  log: (m: string) => void,
): Promise<number> {
  if (!gameLogEnabled(ctx)) return 0;
  ...
  if (tail.rotated) log('gamelog: the log rotated, reading from the start');
 ...
 */
export async function skipToEnd(ctx: Ctx): Promise<void> {
  const tail = await ctx.mod.tailFile(GAME_LOG_PATH, 0);
  if (tail) setOffset(ctx, tail.at);
}
