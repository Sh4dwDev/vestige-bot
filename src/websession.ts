...
export const clearCookie = (
  ...
  secure: boolean,
  sameSite?: 'Lax' | 'None',
): string => serializeCookie(name, '', sameSite
...
export function readSession(
  ...
  if (version !== 'v1' || !discordId || !expiresRaw || !signature) return null;
  if (!/^\d{5,25}$/.test(discordId)) return null;

  const expiresAt = Number.parseInt(expiresRaw, 10);
  if (!Number.isFinite(expiresAt) || expiresAt <= now) return null;

  if (!sameSignature(sign(`v1.${discordId}.${expiresRaw}`, secret), signature)) return null;
