 ...
 */
const LOOK_PATTERNS: Record<string, number> = {
...
export const BUILT_IN: Record<string, Look> = Object.fromEntries(
  Object.entries(LOOKS).map(([name, colours]) => [
    name,
 ...
 */
export const WHOLE_LOOK = { theme: 0, variation: 0 } as const;
...
export async function applyLookIndexes(
  ...
  if (look.theme !== undefined) args['theme'] = look.theme;
  if (look.variation !== undefined) args['variation'] = look.variation;

  // Nothing to change is a success, not a round trip. The mod answers such a
  // call with "read only, nothing was asked for" anyway.
  if (Object.keys(args).length === 0) return true;

  try {
    const result = await ctx.mod.run('look', steamId, args, { quiet: true });
    return result.ok;
  } catch {
...
export async function captureBaseline(
  ...
  if (ctx.db.baselineFor(steamId, species)) return;

  try {
    const read = await ctx.mod.run('skinget', steamId, {}, { quiet: true });
    if (!read.ok) return;

    const live = (read.data ?? {}) as unknown as Record<string, unknown>;
    const colours: Record<string, string> = {};
    for (const [field, rgb] of Object.entries(live)) {
      if (Array.isArray(rgb) && rgb.length === 3) {
        colours[field] = linearToHex(rgb[0] as number, rgb[1] as number, rgb[2] as number);
      }
    }
